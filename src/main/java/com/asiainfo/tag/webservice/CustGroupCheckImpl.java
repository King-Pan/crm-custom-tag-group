package com.asiainfo.tag.webservice;


import com.asiainfo.tag.service.CustGroupService;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Qualifier;
import org.springframework.stereotype.Component;

import javax.jws.WebService;

/**
 * @author king-pan
 */

@Slf4j
@Component
@WebService(serviceName = "custgroupcheck", targetNamespace = "http://service.asiainfo.com", endpointInterface = "com.asiainfo.tag.webservice.CustGroupCheck")
public class CustGroupCheckImpl implements CustGroupCheck {

    @Autowired
    @Qualifier("custGroupServiceImpl")
    private CustGroupService custGroupService;

    @Override
    public String checkExistsNo(String jsonParam) {
        log.info("查询某号码是否在客户群内参数: [" + jsonParam + "]");
        long startTime = System.currentTimeMillis();
        String result = custGroupService.checkTagInGroup(jsonParam);
        long endTime = System.currentTimeMillis();
        log.info("查询某号码是否在客户群内,返回结果: " + result);
        log.info("查询某号码是否在客户群内共耗时: " + (endTime - startTime));
        return result;
    }

    @Override
    public String getCustGroupList(String jsonParam) {
        log.info("查询某号码所有客户群-->参数: [" + jsonParam + "]");
        long startTime = System.currentTimeMillis();
        String result = custGroupService.getUserTagGroupList(jsonParam);
        log.info("查询某号码所有客户群 返回结果:{}", result);
        long endTime = System.currentTimeMillis();
        log.info("查询某号码所有客户群共耗时: " + (endTime - startTime));
        return result;
    }
}
